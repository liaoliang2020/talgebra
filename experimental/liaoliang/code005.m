...
function code001_sub(given_rank)
	...
	for slice_index = 1: prod(fft_tsize)
		disp([slice_index, prod(fft_tsize)]);
	...
	file_name_full = sprintf('%s\\%s', pwd, 'code005.txt');
	fid = fopen(file_name_full, 'a+');
	fprintf('N = %05d \t rank = %05d \t PSNR_value = %f \n', N, given_rank, PSNR_value);
	fprintf(fid, 'N = %05d \t rank = %05d \t PSNR_value = %f \n', N, given_rank, PSNR_value);
	fclose(fid);
